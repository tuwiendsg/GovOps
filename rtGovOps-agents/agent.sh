...
LB_HOST="http://128.130.172.231:8080/SDGBalancer/balancer"
...
then
	...
	if [ $TMP == 8 ]
		...
		then
		  ...
		  finishError "Could not connect to load balancer: $TMP"
		else 
...
# Register container (CoreOS IP and its forwarded port)
#HOST_PORT=`uname -n`
META="location=gh1&type=FM5300"
REGISTRATION_URL="$BASEURL/registerMeta/$MAC/$META"
#echo "MY IP:PORT is $COREOS_PRIVATE_IPV4:$HOST_PORT"
#wget -O - $REGISTRATION_URL --post-data "$COREOS_PRIVATE_IPV4:$HOST_PORT"
